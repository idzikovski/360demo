...
namespace SmartHotel.Clients.Core.ViewModels
{
    public class BookingCalendarViewModel : ViewModelBase
    {
        ...
        void SelectedDate(object date)
        {
            ...
            if (Dates.Any())
            {
                var sorted = Dates.OrderBy(d => d.Year).ThenBy(d => d.Month).ThenBy(d => d.Day);

                From = sorted.FirstOrDefault();
                Until = sorted.LastOrDefault();
                IsNextEnabled = Dates.Any() ? true : false;

                System.Diagnostics.Debug.WriteLine($"First: {From}");
                System.Diagnostics.Debug.WriteLine($"Last: {Until}");
            }
        }
